...
printout(paste('Reading the WFU raw pedigree:', wfu_raw_ped))
wfu_ped <- read_wfu_raw_ped(wfu_raw_ped)
colnames(wfu_ped) <- gsub('.', '', colnames(wfu_ped), fixed=T)

# initialize ID map for WFU pedigree samples
...
printout(paste('Reading the HSW raw pedigree:', wfu_raw_ped))
...
# remove samples from shihpping sheets if they are already incorporated into pedigrees
